...
class customer:
    ...
    def updatemodifydata(self,company_id):
        ...
        try:
            ...
            return None
    #联系量统计
    def get_tongjicontact(self,user_id="",rank="",contacttype="",telflag="",fromdate="",todate="",teltags="",tomorow=""):
        ...
        if dotype:
            if dotype[0:3]=="vap":
                ...
                if result:
                    return 1
    #客户数统计
    def get_tongjicompany(self,user_id="",rank="",nocontact="",gendiu="",user_category_code="",vapflag=""):
    ...
    def get_tongjicompanylist(self,frompageCount="",limitNum="",user_id="",rank="",nocontact="",gendiu="",user_category_code="",vapflag=""):
        indexname="company delta_company"
        servername=searchconfig['servername']
        serverport=searchconfig['serverport']
        cl=SphinxClient()
    ...
    def get_allcustomer(self,frompageCount="",limitNum="",searchlist="",user_id=""):
        ...
        if searchlist:
            ...
            #曾4星 \5星客户
            if isvap:
                if str(searchlist.get("vap_is4star"))=="1":
                    cl.SetFilter('vap_is4star',[1])
                if str(searchlist.get("vap_is5star"))=="1":
        ...
        if res:
            if res.has_key('matches'):
                ...
                for match in itemlist:
                    ...
                    list={'id':id,'name':name,'checked':checked,'star':star,'last_login_time':last_login_time,'regtime':regtime,'email':email,'num_login':num_login,'contact':contact,'tel_country_code':tel_country_code,'tel_area_code':tel_area_code,'tel':tel,'mobile':mobile,'area_code':area_code,'areatxt':areatxt,'salesman':salesman,'assigntime':assigntime,'emphases':emphases,'sqman':sqman,'vap_salesman':vap_salesman,'vap_assigntime':vap_assigntime,'vap_emphases':vap_emphases,'visit_count':visit_count,'salesinfo':salesinfo,'servicetime':servicetime,'adminusername':adminusername,'account':account,'rank':rank,'lastgonghai':lastgonghai,'user_category_code':user_category_code}
                    listall.append(list)
            listcount=res['total_found']
    ...
    def getguowailist(self):
        ...
        return result
    #获得所有销售人员
    def get_allsalesman(self,user_id='',vap=""):
        qxlist=self.getuserqx(user_id)
        qx=""
        for l in qxlist:
            qx+=str(l)+","
